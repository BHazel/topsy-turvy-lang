...
/// &lt;name&gt;</c> syntax dispatched on the referenced variable recorded type. Control-flow statements are
...
public sealed class TopsyTurvyToUtopIRTransformer(ITemporaryVariableNameFormatter? formatter = null, ILabelNameFormatter? labelFormatter = null)
{
    ...
    /// <summary>
    /// The map from array variable name to its declared element <see cref="UtopIRType"/>, keyed
    /// separately from <c>declaredTypes</c> since that map only ever records the
    /// <see cref="UtopIRType.Array"/> marker for an array variable, not its true element type.
    /// </summary>
    /// <remarks>
    /// This is similar to the <c>TypeCheckVisitor.arrayElementTypeStack</c> side-table pattern for the Topsy Turvy
    /// array type. Reset at the start of every <see cref="Transform"/> call, and instance-scoped rather
    ...
    /// </summary>
    /// <remarks>
    /// Casts the value to the array declared element type when needed, mirroring
    ...
    /// <see cref="TransformExpression"/> only dispatches here when <paramref name="declaredTypes"/>
    /// records the <paramref name="arrayIndex"/> <see cref="ArrayIndexNode.ArrayName"/> as
    /// <see cref="UtopIRType.Array"/>; the temporary register true type is looked up from
    /// <see cref="arrayElementTypes"/>, since <paramref name="declaredTypes"/> only ever records the
