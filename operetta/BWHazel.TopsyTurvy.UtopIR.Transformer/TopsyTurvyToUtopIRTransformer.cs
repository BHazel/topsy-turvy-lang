...
/// <c>ONCE MORE.</c> continue), top-level programme return, and <c>yarn</c> character access via
/// <c>VICTIM &lt;index&gt; ON &lt;yarn&gt;</c>. Array element access via the same <c>VICTIM</c> syntax
/// on an array variable is not yet supported. Control-flow statements are
...
public sealed class TopsyTurvyToUtopIRTransformer(ITemporaryVariableNameFormatter? formatter = null, ILabelNameFormatter? labelFormatter = null)
{
    ...
    /// <c>yarn</c> character access and array element access.  <see cref="TransformExpression"/> only
    /// dispatches here when <paramref name="declaredTypes"/> records the <paramref name="arrayIndex"/>
    /// <see cref="ArrayIndexNode.ArrayName"/> as <see cref="UtopIRType.Yarn"/>.
    /// </remarks>
