...
/// <summary>
/// Tests for the <see cref="ToolchainExports"/> class.
/// </summary>
/// <remarks>
...
/// pointer must be obtained via <c>&amp;ToolchainExports.Method</c> and invoked through that, exactly as a
...
/// managed types.
/// </remarks>
public class ToolchainExportsTests
{
    /// <summary>
    /// Tests that <see cref="ToolchainExports.ApiVersion"/> returns the frozen contract version.
    /// </summary>
    [Fact]
    public void ApiVersion_WhenCalled_ReturnsTwo()
    {
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.CreateSession"/> followed by <see cref="ToolchainExports.DestroySession"/> does not throw.
    /// </summary>
    [Fact]
    public void CreateSession_ThenDestroySession_DoesNotThrow()
    {
        ...
        Should.NotThrow(() => NativeExportsTestSupport.DestroySession(session));
    }

    /// <summary>
    /// Tests that <see cref="ToolchainExports.DestroySession"/> given an invalid or already-destroyed handle does not throw.
    /// </summary>
    [Fact]
    public void DestroySession_WithInvalidOrDoubleDestroyedHandle_DoesNotThrow()
    {
        ...
        Should.NotThrow(() => NativeExportsTestSupport.DestroySession(session));
        Should.NotThrow(() => NativeExportsTestSupport.DestroySession(0));
        Should.NotThrow(() => NativeExportsTestSupport.DestroySession(12345));
    }

    /// <summary>
    /// Tests that <see cref="ToolchainExports.AnalyseSource"/> reports success with no diagnostics for a valid programme.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.AnalyseSource"/> reports a diagnostic with a positive line and column for an unterminated string literal.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.AnalyseSource"/> tracks a surrogate-pair emoji in a <c>YARN</c> literal as
    ...
    {
        nint asciiSession = NativeExportsTestSupport.CreateSession();
        nint emojiSession = NativeExportsTestSupport.CreateSession();
        ...
        {
            NativeExportsTestSupport.DestroySession(asciiSession);
            NativeExportsTestSupport.DestroySession(emojiSession);
        }
    }

    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetHover"/> returns Markdown content for a symbol at the requested position.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetCompletions"/> offers keyword candidates at the start of an empty document.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetCompletions"/> filters keyword items to a multi-word namespace keyword when its own first words are typed as the whole phrase.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetCompletions"/> filters keyword items to <c>WITH DISTRICT</c> and not the shorter, pre-existing bare <c>WITH</c> keyword.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.FormatSource"/> normalises keyword casing to the canonical uppercase form.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetTokens"/> reports a single "comment" token spanning multiple lines for a block comment, including one left unterminated at end-of-source.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetTokens"/> categorises a string literal, a keyword phrase, a
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.ExecuteProgramme"/> invokes the registered output callback for a <c>BEHOLD</c> statement.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.ExecuteProgramme"/> declares <c>THE PROPS</c> from the arguments passed in <c>args_json_utf8</c>.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.ExecuteProgramme"/> can run the same session declarations twice without an "already declared" runtime error.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.CancelExecution"/> stops a running infinite loop and that
    /// <see cref="ToolchainExports.GetLastError"/> then reports the cancellation.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that <see cref="ToolchainExports.GetLastError"/> returns no message for a session with no recorded error.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that in <see cref="ToolchainExports.ExecuteProgramme"/> <c>PRAY TELL</c> falls through to the
    /// registered <c>input_line</c> callback once the pre-supplied stdin queue is empty, matching
    /// the <see cref="BufferedNativeIO"/> three-step <c>ReadLine</c> chain.
    /// </summary>
    [Fact]
    public void ExecuteProgramme_WithPrayTellAndNoStdinButRegisteredInputLine_ReadsFromInputCallback()
    {
        TestCallbackCapture.Reset();
        TestCallbackCapture.SetInputLineResult("callback-supplied line");
        nint session = NativeExportsTestSupport.CreateSession();
        string source = "HARK! \"Test\"\n\nPRINCIPALS\nPRAY WELCOME Line AS A YARN\nTHE CURTAIN RISES.\n\nPRAY TELL Line\nBEHOLD Line\n\nFINALE.\n";

        try
        {
            int status = Execute(session, source);

            status.ShouldBe(0);
            TestCallbackCapture.OutputLines.ShouldContain("callback-supplied line");
        }
        finally
        {
            NativeExportsTestSupport.DestroySession(session);
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.AnalyseSource"/> and deserialises its result.
    /// </summary>
    ...
    {
        delegate* unmanaged<nint, byte*, byte*> analyseSource = &ToolchainExports.AnalyseSource;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.GetHover"/> and deserialises its result.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    ...
    {
        delegate* unmanaged<nint, byte*, int, int, byte*> getHover = &ToolchainExports.GetHover;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.GetCompletions"/> and deserialises its result.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    /// <param name="line">The 0-indexed line number.</param>
    ...
    {
        delegate* unmanaged<nint, byte*, int, int, byte*> getCompletions = &ToolchainExports.GetCompletions;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.GetTokens"/>.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    ...
    {
        delegate* unmanaged<nint, byte*, byte*> getTokens = &ToolchainExports.GetTokens;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.FormatSource"/>.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    ...
    {
        delegate* unmanaged<nint, byte*, byte*> formatSource = &ToolchainExports.FormatSource;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.ExecuteProgramme"/> with no arguments or pre-seeded input.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.ExecuteProgramme"/> with a JSON-encoded <c>THE PROPS</c> argument array.
    /// </summary>
    /// <param name="session">The session handle.</param>
    /// <param name="source">The Topsy Turvy source text.</param>
    ...
    {
        delegate* unmanaged<nint, byte*, byte*, byte*, int> executeProgramme = &ToolchainExports.ExecuteProgramme;
    ...
    /// <summary>
    /// Calls <see cref="ToolchainExports.CancelExecution"/>.
    /// </summary>
    /// <param name="session">The session handle.</param>
    private static unsafe void Cancel(nint session)
    {
        delegate* unmanaged<nint, void> cancelExecution = &ToolchainExports.CancelExecution;
        cancelExecution(session);
    }

    /// <summary>
    /// Calls <see cref="ToolchainExports.GetLastError"/>.
    /// </summary>
    /// <param name="session">The session handle.</param>
    ...
    {
        delegate* unmanaged<nint, byte*> getLastError = &ToolchainExports.GetLastError;
        ...
        NativeExportsTestSupport.FreeBuffer(errorPointer);
        return message;
    }

    /// <summary>
    /// Calls <see cref="ToolchainExports.ApiVersion"/>.
    /// </summary>
    /// <returns>The native export contract version.</returns>
    private static unsafe int ApiVersion()
    {
        delegate* unmanaged<int> apiVersion = &ToolchainExports.ApiVersion;
        return apiVersion();
    }
}
