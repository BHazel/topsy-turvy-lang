#!/usr/bin/env bash
#
# Publishes BWHazel.TopsyTurvy.Embedded for iOS and macOS runtime identifiers and combines the two dylib
# slices into a single XCFramework for hand-rolled Xcode consumption.
...
RIDS=("ios-arm64" "iossimulator-arm64" "osx-arm64")
...
OSX_ARM64_DYLIB="$(resolve_dylib osx-arm64)"

# Verify that all dylibs were found, otherwise exit with an error.
if [[ -z "${IOS_ARM64_DYLIB}" || -z "${IOSSIMULATOR_ARM64_DYLIB}" || -z "${OSX_ARM64_DYLIB}" ]]; then
...
# Create the XCFramework from the three dylib slices and their headers.
xcodebuild -create-xcframework \
    -library "${IOS_ARM64_DYLIB}" -headers "${HEADERS_DIR}" \
    -library "${IOSSIMULATOR_ARM64_DYLIB}" -headers "${HEADERS_DIR}" \
    -library "${OSX_ARM64_DYLIB}" -headers "${HEADERS_DIR}" \
