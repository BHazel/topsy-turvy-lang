...
* **Baseline:** `dotnet build interpreter/BWHazel.TopsyTurvy.slnx --no-incremental` — 0 errors, 0 warnings. `dotnet test interpreter/BWHazel.TopsyTurvy.Tests` — 472/472. `dotnet test interpreter/BWHazel.TopsyTurvy.Cli.E2ETests` — 50/50. `dotnet test interpreter/BWHazel.TopsyTurvy.WebEditor.E2ETests` — 6/6 Playwright. `npm run test:grammar` — 4 files/all passing. `npm run test:unit` — 4/4.
...
| `interpreter/BWHazel.TopsyTurvy.Runtime/` | Tree-walking interpreter. Entry: `Interpreter`. Key types: `TopsyTurvyValue`, `TopsyTurvyEnvironment` (now tracks constants via `HashSet<string>`; `Declare` accepts optional `isConstant`; `Assign` enforces the constant constraint; `IsConstant(name)` query added), `ITopsyTurvyIO`/`ConsoleIO`. Signal exceptions: `BreakSignalException`, `ContinueSignalException`, `ReturnSignalException`, `TopsyTurvyThrowException`. `Execute(program, cancellationToken, options?)` — pass `InterpreterExecutionOptions.ExecutionTimeout` for Blazor WASM timeout (see §2). `GenerateDocumentationFile` is enabled; XML documentation **complete** for all types. |
