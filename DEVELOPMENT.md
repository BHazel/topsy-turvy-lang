...
* **Last Updated:** 2026-06-16 (Language Server XML documentation **complete** — all 16 handlers and 2 support files (`LanguageServerConstants`, `LspUtilities`) documented and reviewed. Review identified: `"symbol namem"` typo in `RenameHandler.cs` Handle remarks; missing space before `<c>` tag in `HoverHandler.cs`; minor formatting issues in `CompletionHandler.cs`, `DocumentFormattingHandler.cs`, `DefinitionHandler.cs`, `DocumentSymbolHandler.cs`. `dotnetmd` identified as cause of LSP handler pages missing from Docusaurus API docs — see §4 Known Gaps. **Previously:** `LanguageServerConstants`, `LspUtilities`, `DocumentStateManager`, `TextDocumentSyncHandler`, `DocumentSymbolHandler` documented. `LspUtilities.BuildLocation` renamed to `GetSymbolDefinitionLocation`; callers in `DocumentSymbolHandler` and `WorkspaceSymbolHandler` updated. Hierarchical `DocumentSymbol` format added to Deferred LSP Features as upgrade path for the current flat `SymbolInformation` approach. **Previously:** Analysis layer XML documentation **complete** — all 9 types documented. `"implicit accumulator"` corrected to `"implicit variable"` in `SymbolTable.cs`, `KeywordData.cs`, `HoverMarkdownBuilderTests.cs`, `topsy-turvy-language.js`, and `AGENTS.md`. Two new Known Gaps added: flat `SymbolTable` namespace causing parameter name collisions across functions; AST source positions not wired into the parser (PlaceholderSpan) — moved from Non-Obvious Constraints. `HoverMarkdownBuilder.cs` line 19 em dash restored — `Build_WithJustSo_ReturnsImplicitVariableMarkdown` passing. **Pending fix:** `SourceAnalyser.cs` has `"occurences"` typo in `FindWordOccurrences` `<remarks>` and `CountOccurrences` `<summary>` — correct to `"occurrences"`.)
...
| `interpreter/BWHazel.TopsyTurvy.Ast/` | AST node types. All inherit from `Node` (`required SourceSpan Span`). Also defines `Diagnostic`, `DiagnosticCollection`, `SourceSpan`, `SourceLocation`. `GenerateDocumentationFile` is enabled; XML documentation comments are present on all types. |
| `interpreter/BWHazel.TopsyTurvy.Analysis/` | Shared analysis library. `SymbolTable`, `SymbolInfo`, `SymbolKind`, `DocumentState`, `DepthAction`. `SourceFormatter` — keyword normalisation + libretto indentation. `SourceAnalyser` — `FindSkipRanges`, `BuildLineOffsets`, `IsInSkipRange`, `FindWordOccurrences`, `CountOccurrences`. `KeywordData` — 74-entry keyword list (authoritative source of truth for LSP/completion — must not be duplicated in `Keywords.cs`). `HoverMarkdownBuilder` — Markdown hover strings. References: Ast, Parser. Referenced by: LanguageServer, Cli, WebEditor. `GenerateDocumentationFile` is enabled; XML documentation **complete** for all types. |
| `interpreter/BWHazel.TopsyTurvy.Parser/` | Superpower parser. Entry: `TopsyTurvyParser`. Pre-processors: `CommentsPreProcessor`, `VictorianFlourishPreProcessor`. `SourceMap`/`SourceMapping` for offset tracking (`SourceMapping.TransformedOffset` maps a position in the pre-processed text to its original line/column). Pre-processor output carried in `PreProcessResult(TransformedText, SourceMap)`. `TryParse` returns `ParseResult(ProgramNode?, IReadOnlyList<Diagnostic>)`. `GenerateDocumentationFile` is enabled; XML documentation **complete** for all types. |
...
| `interpreter/BWHazel.TopsyTurvy.Runtime/` | Tree-walking interpreter. Entry: `Interpreter`. Key types: `TopsyTurvyValue`, `TopsyTurvyEnvironment`, `ITopsyTurvyIO`/`ConsoleIO`. Signal exceptions: `BreakSignalException`, `ContinueSignalException`, `ReturnSignalException`, `TopsyTurvyThrowException`. `Execute(program, cancellationToken, options?)` — pass `InterpreterExecutionOptions.ExecutionTimeout` for Blazor WASM timeout (see §2). `GenerateDocumentationFile` is enabled; XML documentation **complete** for all types. |
...
| `interpreter/BWHazel.TopsyTurvy.WebEditor/wwwroot/js/web-editor.js` | Monaco/xterm JS interop — hover, completion, definition providers, terminal fit, download utilities. Extends `window.topsyTurvy` via `Object.assign`. |
...
**`AS IT WERE` is statement-only (deferred)**: `ExpressionCastNode` extends `Statement`, and `ExpressionParser.Expression` has no `AS IT WERE` alternative. This means the cast cannot appear inline (e.g. `BEHOLD AS IT WERE x AS A YARN` fails at parse time). The intended inline fix is to make `ExpressionCastNode` extend `Expression` and add an `AS IT WERE` branch to `ExpressionParser.Expression`; until then the two-step idiom (`AS IT WERE x AS A YARN` / `BEHOLD JUST SO`) is the workaround.
