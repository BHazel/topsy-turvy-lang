...
* **Last Updated:** 2026-06-13 (Parser layer XML documentation **complete** — all types documented including `TopsyTurvyParser`, `ParseResult`, `TopsyTurvySyntaxException`, and `LoopDefinition`. `FindDeclarationSpan` in `TopsyTurvyParser` is a known drift point: its three hardcoded regex patterns mirror parser keywords and will silently fall back to span `(1,1)-(1,2)` if a keyword is renamed without updating this method. It is a temporary workaround until `PlaceholderSpan` is replaced with real AST positions, at which point `declaration.Span` can be used directly and the method removed. `BuildPreProcessorPipeline` extracted as a shared private factory used by both `Parse` and `TryParse`. `Parse` vs `TryParse`: `Parse` is for fatal-error callers (CLI); `TryParse` returns structured `ParseResult` for callers that need the AST and diagnostics simultaneously (LSP, web editor). Refactoring: `SingleAndExpression` extracted in `ExpressionParser`; `PlaceholderSpan` and `Ws<T>()` moved into `ParserHelpers`; `ConditionalBodyInfo` and `SwitchBodyInfo` records added. Breaking renames: `PreProcessResult.Text` → `TransformedText`; `SourceMapping.PreProcessedOffset` → `TransformedOffset`. Spec: §10 loop label clarification — `KNOWN AS <label>` is readability only.)
...
| `interpreter/BWHazel.TopsyTurvy.Analysis/` | Shared analysis library. `SymbolTable`, `SymbolInfo`, `SymbolKind`, `DocumentState`, `DepthAction`. `SourceFormatter` — keyword normalisation + libretto indentation. `SourceAnalyser` — `FindSkipRanges`, `BuildLineOffsets`, `IsInSkipRange`, `FindWordOccurrences`, `CountOccurrences`. `KeywordData` — 74-entry keyword list. `HoverMarkdownBuilder` — Markdown hover strings. References: Ast, Parser. Referenced by: LanguageServer, Cli, WebEditor. |
...
| `interpreter/BWHazel.TopsyTurvy.LanguageServer/` | OmniSharp LSP **class library** — see §3. Started by the CLI via `operetta sorcerer incantation`. `LspUtilities` — shared `BuildLocation` and `MapSymbolKind`. `DocumentStateManager` — singleton; exposes `FindSymbolInOtherDocuments` and `GetImportedFunctionSymbols` for cross-document lookups. |
